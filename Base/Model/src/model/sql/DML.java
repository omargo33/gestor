...
public class DML
{
    private static String[] PALABRAS_RESERVADAS_INYECCION_SQL = new String[] { "SELECT", "FROM", "WHERE", "INSERT", "INTO", "IN TO", "UPDATE", "DELETE", "FROM", "/*", "--", "*/", "CA_USUARIOS", "CLIENTES", "SOCIOS" };
  ...
  public synchronized ResultSet ejecutaConsulta(String sqlConsulta, Object... parametros) {
    ...
    try {
        this
          ...
          .preparedStatement = getDBTransaction().createStatement(-1).getConnection().prepareStatement(sqlConsulta, 1004, 1007);
  ...
  private void closePreparedStatement() {
    ...
    public void addEach(SobrecargaAcciones documentTemp) { this.sobrecargaAcciones = documentTemp; }
  ...
  private void insercionTipoDatoPreparedStatemet(int posicion, Object dataIsert) throws SQLException {
      ...
      if (nombreObjeto.equalsIgnoreCase("java.sql.Clob")) {
        this.preparedStatement.setClob(posicion, (Clob)dataIsert);
    }
    
      if (nombreObjeto.equalsIgnoreCase("java.sql.Date")) {
        this.preparedStatement.setDate(posicion, (Date)dataIsert);
    }
    
      if (nombreObjeto.equalsIgnoreCase("java.util.Date")) {
        Date fechaTemp = new Date(((Date)dataIsert).getTime());
  ...
  private String controlInyeccionSQL(String parametro) {
    ...
    public void commit() { getDBTransaction().commit(); }
    ...
    public void rollback() { getDBTransaction().rollback(); }
  ...
  public boolean isClosed() {
    ...
    public ResultSet getCursor() { return this.cursor; }
    ...
    public DBTransaction getDBTransaction() { return null; }
}
